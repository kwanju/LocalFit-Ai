...
- **핵심 변경**:
  ...
  - TTS = **Qwen3-TTS + SDPA 어텐션 + sentence-streaming** (ADR-006) — flash-attn 없이 97ms 지연
  - 능동 코치 = **instructor + JSON 구조화 출력** (ADR-013) — 사용자 발화로 카운팅 자동 시작
  - 카운팅 = 메트로놈/타이머 유지 + LLM `start_counting` 액션 트리거 (ADR-014)
...
1. `docs/prd-v4.md` — 제품 요구사항. **단일 진실 소스**
2. `docs/architecture/adr/README.md` — 19개 ADR 인덱스
3. 현재 작업과 **직접 관련된 ADR만** (전부 읽지 말 것 — 컨텍스트 절약)
4. `docs/conventions/coding-style.md` — 코딩 규약 (v1 자산 재활용)
