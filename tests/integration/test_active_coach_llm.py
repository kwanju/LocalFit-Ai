...
# ADR-033: 과도한 보수 표현 제거 — 자가보고 수치를 그대로 기준선으로 쓰는 톤으로 갱신.
_FIRST_SESSION_CONTEXT = (
    "사용자: 30대 / 🔰 첫 세션(기준선 없음): 대화로 체력 확인. 온보딩 자가보고 시드: "
    "푸시업 15회, 플랭크 30초 시드가 맞는지 확인한 뒤 사용자가 말한 수치를 그대로 "
    "기준선으로 set_baseline 저장하고, 적당한 첫 세트를 제안."
)
